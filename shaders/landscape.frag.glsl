...
void main() {
    ...
    float temperature = 0.5 - 0.4 * max(length(coord) - 20.0, 0.0);
    temperature += noise8 + 0.6 * noise4 * 0.1 + noise2 * 0.3 + noise1 * 0.07;
    

    float landFactor = smoothstep(-0.05, 0.05, temperature);
    ...
    vec3 landColor = mix(vec3(0.0 + temperature, 0.6, 0.0 + temperature), mountainColor, smoothstep(0.0, 0.1, mountainFactor));
    float grassFactor = smoothstep(0.75, 0.7, elevation);
    landColor = mix(landColor, vec3(0.2 + temperature, 0.8 + temperature, 0.0 + temperature), grassFactor);
    float beachFactor = smoothstep(0.5, 0.4, elevation);
    landColor = mix(landColor, vec3(0.9 * temperature, 0.9 * temperature, 0.5 * temperature), beachFactor);

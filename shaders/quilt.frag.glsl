...
void main() {    
    ...
    if (mod(2.0 * cell.x + cell.y, 5.0) <= (5.0 * seed)) {
        color = 1.0 - color;
    }

    // 3. Uncomment these lines to produce interesting colors
    float r = mod(1.5 * cell.x + 3.0 * cell.y, 7.0) / 7.0;
    float g = mod(2.0 * cell.x + 4.0 * cell.y, 7.0) / 7.0;
    float b = mod(3.0 * cell.x + 5.0 * cell.y, 7.0) / 7.0;
